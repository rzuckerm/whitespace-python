...
from whitespace.interpreter import run
...
class EvalTestCase(unittest.TestCase):
    ...
    def test_it_evaluates_count_ws(self):
        run(src("count"), screen=self.screen)
    ...
    def test_it_evaluates_fact_ws(self):
        ...
        run(src("fact"), keyboard=self.keyboard, screen=self.screen)

        self.assertEqual(
            self.screen.contents,
            "Enter a number: 40! = 815915283247897734345611269596115894272000000000\r\n",
        )

    def test_it_evaluates_hello_ws(self):
        run(src("hello"), screen=self.screen)
    ...
    def test_it_evaluates_name_ws(self):
        ...
        run(src("name"), keyboard=self.keyboard, screen=self.screen)

        self.assertEqual(self.screen.contents, "Please enter your name: Hello Dwayne\n\r\n")
